...
class Client(BaseModelUUID):
    ...
    allowed_uris = Column(JSON, comment="allowed uris")
    ...
    jwt_public_key = Column(String(2000), comment="jwt public key with pem format")
    jwt_private_key = Column(String(2000), comment="jwt private key with pem format")
...
class User(BaseModelUUID):
    ...
    disabled = Column(Boolean, default=False)
    provider = Column(String(30),nullable=True) # google, github, apple, wechat
